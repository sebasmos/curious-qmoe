...
def run_cv(csv_path: str, cfg: DictConfig):
    ...
    # Store peak RAM usage instead of average
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        # Measure memory usage for training by directly calling train_pytorch_local
        mem_train_usage, (model_trained_state, train_losses, val_losses, best_f1_from_train, 
            ...
            memory_usage(
                (train_pytorch_local, (
                    ...
                    in_dim,         # num_columns
                    device,         # device
                    str(fold_dir),  # fold_folder
                    resume,         # resume_checkpoint
                    str(ckpt_path)  # checkpoint_path
                )), 
                interval=0.1, 
                retval=True
            )
        ...
        # Measure memory usage for validation by directly calling _validate_single_epoch
        ...
        fold_result = {
            ...
            "max_ram_mb_train": float(max_ram_mb_train), # Changed to max_ram_mb
            "max_ram_mb_val": float(max_ram_mb_val),     # Changed to max_ram_mb
        }
        ...
        all_max_ram_mb_train.append(max_ram_mb_train) # Append to new list
        all_max_ram_mb_val.append(max_ram_mb_val)     # Append to new list
...
# # --- Wrapper functions for memory_profiler ---
# def _train_model_wrapper(model_instance, args, train_loader, val_loader, class_weights, num_columns, device, fold_folder, resume_checkpoint, checkpoint_path):
#     """Wrapper for train_pytorch_local to be used with memory_usage."""
#     return train_pytorch_local(
#         args=args,
...
# def _validate_model_wrapper(model_instance, val_loader, criterion, device):
#     """Wrapper for _validate_single_epoch to be used with memory_usage."""
#     return _validate_single_epoch(
#         model_instance, val_loader, criterion, device
#     )
...
#         # Measure memory usage for training
#         mem_train_usage, (model_trained_state, train_losses, val_losses, best_f1_from_train, 
#                            all_labels_best_epoch, all_preds_best_epoch, all_probs_best_epoch) = \
#             memory_usage((_train_model_wrapper, (model, cfg.experiment, train_ld, val_ld, class_weights, 
#                                                  in_dim, device, str(fold_dir), resume, str(ckpt_path))), 
...
#         # Measure memory usage for validation
#         mem_val_usage, (_, _, all_labels_final, all_preds_final, all_probs_final) = \
#             memory_usage((_validate_model_wrapper, (final_model, val_ld, torch.nn.CrossEntropyLoss(), device)),
#                          interval=0.1, retval=True)
