...
import pandas as pd
import torch
from sklearn.model_selection import StratifiedKFold # No longer importing train_test_split
...
def run_cv(csv_path: str, cfg: DictConfig):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        _, _, all_labels_final, all_preds_final, all_probs_final = _validate_single_epoch(
            final_model, val_ld, torch.nn.CrossEntropyLoss(), device # Using val_ld here
        )
        
        val_tracker.stop()

        train_stats = _load_cc_csv(fold_dir / "emissions_train.csv") 
        val_stats = _load_cc_csv(fold_dir / "emissions_val.csv") # Load from new val file
        ...
        for k in cc_metrics_to_track:
            fold_result[f"codecarbon_val_{k}"] = val_stats.get(k, 0.0) # Renamed prefix to 'val'
            all_val_cc_data_agg[k].append(val_stats.get(k, 0.0)) # Renamed aggregation list
    ...
    for phase_data_agg, prefix in [(all_train_cc_data_agg, "train"), (all_val_cc_data_agg, "val")]: # Renamed prefix
        for k in cc_metrics_to_track:
            if phase_data_agg[k]:
