...
#     experiment.datasets.esc.csv=/home/sebastian/codes/data/ESC-50-master/VE_soundscapes/efficientnet_1536/esc-50.csv \
#     experiment.device=cuda \
#     experiment.metadata.tag=moe_opt
...
# class MoEModel(nn.Module): # This class seems to be unused, MoEModelBatched is used
...
# class MoEModelBatched(nn.Module):
#     def __init__(self, cfg, in_dim, num_classes, num_experts, top_k): # Added num_experts, top_k as direct args for clarity with Optuna
#         super().__init__()
#         # Updated Router initialization
#         self.router = Router(in_dim, cfg.experiment.router.hidden_dim, num_experts, cfg.experiment.router.dropout_prob)
#         self.experts = nn.ModuleList([
#             ESCModel(in_dim, num_classes,
#                      hidden_sizes=cfg.experiment.model.hidden_sizes, # Use cfg for hidden_sizes
#                      dropout_prob=cfg.experiment.model.dropout_prob)
#             for _ in range(num_experts)
#         ])
#         self.num_classes = num_classes
#         self.num_experts = num_experts
#         self.top_k = top_k
#         # Ensure these are accessed correctly, potentially from the cfg passed in the objective
#         self.load_balancing_alpha = cfg.experiment.router.get("load_balancing_alpha", 10e-3) # Now from router
#         self.load_balancing_enabled = cfg.experiment.router.get("load_balancing_enabled", True) # Now from router
...
# # 2. Modify train_moe_local to accept 'trial'
# def train_moe_local(cfg, model, train_loader, val_loader, class_weights, in_dim, device, fold_dir, ckpt_path, trial: optuna.Trial = None): # Added trial argument
#     optimizer = torch.optim.Adam(model.parameters(), lr=cfg.experiment.router.lr_moe_train)
#     criterion = nn.CrossEntropyLoss(weight=class_weights)
#     best_f1 = 0
#     load_balancing_alpha = cfg.experiment.router.load_balancing_alpha
...
#         # Report intermediate F1 to Optuna for pruning
#         if trial: # Check if trial is available
#             trial.report(f1, epoch)
#             if trial.should_prune():
#                 raise optuna.exceptions.TrialPruned()
...
# # Modify run_experiment to accept 'trial' directly
# def run_experiment(cfg: DictConfig, trial: optuna.Trial = None):
#     # No need for cfg.trial = trial here anymore
#     # The 'trial' object is passed directly to functions that need it.
...
#         # Pass the 'trial' object directly to train_moe_local
#         mem_train_usage, best_state = \
#             memory_usage((train_moe_local, (cfg, model, train_ld, val_ld, class_weights, in_dim, device, str(fold_dir), ckpt_path, trial)), interval=0.1, retval=True)
#         # Note the added 'trial' argument here ^^^
...
#         if not trial: # Plot only if not in an Optuna trial
#             plot_multiclass_roc_curve(all_labels_final, all_probs_final, EXPERIMENT_NAME=str(fold_dir))
#             plot_losses(train_losses, val_f1_scores_history, str(fold_dir))
...
#     # if not trial: # Save summary only if not in an Optuna trial
...
#     return summary["f1_mean"] # Return the metric to optimize

# # --- Optuna Objective Function ---
# # This function will now receive the *base* config directly from hydra.main
# def objective(trial: optuna.Trial, base_cfg: DictConfig) -> float:
#     # Make a deep copy of the base configuration for this trial
#     cfg = OmegaConf.create(OmegaConf.to_container(base_cfg, resolve=True))

#     # Override/suggest parameters using Optuna trial
#     cfg.experiment.router.num_experts = trial.suggest_int("num_experts", 2, 6)
#     cfg.experiment.router.top_k = trial.suggest_int("top_k", 1, min(cfg.experiment.router.num_experts, 3))
#     cfg.experiment.router.hidden_dim = trial.suggest_categorical("router_hidden_dim", [64, 128, 256])
#     cfg.experiment.router.dropout_prob = trial.suggest_float("router_dropout_prob", 0.0, 0.4, step=0.1)
#     cfg.experiment.router.lr_moe_train = trial.suggest_loguniform("lr_moe_train", 1e-4, 1e-2)
#     cfg.experiment.router.load_balancing_alpha = trial.suggest_loguniform("load_balancing_alpha", 1e-4, 0.1)
#     cfg.experiment.router.load_balancing_enabled = trial.suggest_categorical("load_balancing_enabled", [True, False])
...
#     cfg.experiment.model.dropout_prob = trial.suggest_float("expert_dropout_prob", 0.0, 0.4, step=0.1)
#     cfg.experiment.model.epochs = trial.suggest_int("epochs", 15, 40)
#     cfg.experiment.model.batch_size = trial.suggest_categorical("batch_size", [32, 64])

#     cfg.experiment.cross_validation.n_splits = trial.suggest_categorical("n_splits", [3, 5])

#     # Ensure a unique tag for each trial's output
#     cfg.experiment.metadata.tag = f"optuna_moe_trial_{trial.number}"
...
# # --- Main Hydra entry point that wraps the Optuna study ---
# @hydra.main(version_base=None, config_path="config", config_name="esc50")
# def main(cfg: DictConfig):
#     warnings.filterwarnings("ignore", category=UserWarning)
...
#     try:
#         # Pass the base_cfg to the objective function using a lambda
#         study.optimize(lambda trial: objective(trial, cfg), n_trials=n_trials, show_progress_bar=True)
#     except KeyboardInterrupt:
#         print("Optimization interrupted by user.")
...
#     # You can also run the best configuration here if desired,
#     # just like in your original commented-out block.
#     # To do that, you'd reconstruct the best_cfg from trial.params
#     # and call run_experiment(best_cfg, None)


# if __name__ == "__main__":
#     main() # Call the hydra_main wrapped function