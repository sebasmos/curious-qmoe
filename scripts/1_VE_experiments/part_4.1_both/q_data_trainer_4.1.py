...
def quantized_collate(batch):
            features = torch.stack([item[0] for item in batch])  # Preserves quantized tensors
...
def main(args):
    for seed in range(args.total_num_seed):
        ...
        print(f'SEED {seed}')
        print('--------------------------------')
        print(f'part_3.1_quantized_data'.center(60,"-"))
        ...
        experiment_folder += "_quantized_both4.1"
        ...
        train_dataset = QuantizedEmbeddingDatasetv31(train_csv, train=True, save_embeddings=False)
        val_dataset = QuantizedEmbeddingDatasetv31(val_csv, train=False, save_embeddings=False)
        ...
        train_loader = DataLoader(
            ...
            shuffle=True,
            collate_fn=quantized_collate  # Preserve quantized tensors
        )
        val_loader = DataLoader(
            val_dataset,
            batch_size=args.batch_size,
            shuffle=False,
            collate_fn=quantized_collate
        )
            # Class weights
        ...
        num_columns = train_dataset.features.size()[1] 
        print("num columns: ",num_columns)
        model = QClassifier(num_columns, args.num_classes, hidden_sizes=[256, 128, 64]).to(device)
