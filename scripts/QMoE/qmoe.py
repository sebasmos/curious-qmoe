...
# ### NEW: Corrected BitLinear implementation based on the provided reference ###
class BitLinear(nn.Module):
    """
    A BitLinear layer that handles various bit-widths (1, 2, 4, 16) and
...
class MoEModelBatched(nn.Module):
    def __init__(self, cfg, in_dim, num_classes, num_experts=4, top_k=2):
        ...
        # --- MODIFIED: Use the new BitNetExpert with specified bit-widths ---
        expert_quantizations = [1, 2, 4, 16] # Bit-widths for each expert
...
# The rest of the script remains unchanged as the training/evaluation logic is sound.
def train_moe_local(cfg, load_balancing, model, train_loader, val_loader, class_weights, in_dim, device, fold_dir, resume, ckpt_path):
    ...
    for epoch in range(cfg.experiment.model.epochs):
        ...
        if accuracy > best_accuracy + (delta if early_stopping_enabled else 0):
...
#     --config-name esc50 \
#     experiment.datasets.esc.normalization_type=standard \
#     experiment.router.lr_moe_train=1e-3 \
#     experiment.datasets.esc.csv=/home/sebastian/codes/data/ESC-50-master/VE_soundscapes/efficientnet_1536/esc-50.csv \
#     experiment.device=cuda \
#     experiment.metadata.tag=qmoe
...
#         # Quantize weights
#         weight_quant = self.quantize(self.weight, self.num_bits)
#         return F.linear(x, weight_quant, self.bias)

#     @staticmethod
#     def quantize(tensor, num_bits):
#         if num_bits == 1: # Bitnet-style quantization
#             return tensor.sign()
...
# # --- NEW QUANTIZED EXPERT MODEL ---
# class QuantizedESCModel(nn.Module):
#     def __init__(self, in_dim, num_classes, hidden_sizes, dropout_prob, num_bits=16):
#         super().__init__()
#         layers = []
#         last_dim = in_dim
#         for hidden_dim in hidden_sizes:
#             layers.append(QuantizedLinear(last_dim, hidden_dim, num_bits=num_bits))
#             layers.append(nn.ReLU())
#             layers.append(nn.Dropout(dropout_prob))
#             last_dim = hidden_dim
#         layers.append(QuantizedLinear(last_dim, num_classes, num_bits=num_bits))
#         self.net = nn.Sequential(*layers)
...
#         # --- MODIFIED: Initialize experts with different quantization levels ---
#         expert_quantizations = [1, 1, 1, 1] # Bit-widths for each expert
#         self.experts = nn.ModuleList([
#             QuantizedESCModel(in_dim, num_classes, 
#                               hidden_sizes=cfg.experiment.model.hidden_sizes, 
#                               dropout_prob=cfg.experiment.model.dropout_prob,
#                               num_bits=bit_width)
#             for bit_width in expert_quantizations
