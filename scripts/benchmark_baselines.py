...
def run_cv(csv_path: str, cfg: DictConfig):
    ...
    for model_kind in to_run:
        ...
        for fold, (tr_idx, vl_idx) in enumerate(skf.split(np.zeros(len(labels)), labels), 1):
            ...
            t0_val = time.perf_counter(); tracker_val.start()
            ...
            tracker_val.stop()
            dur_val = time.perf_counter() - t0_val
            ...
            fold_result = {
                ...
                "train_dur_s": dur_tr,
                "val_dur_s": dur_val,
                "params": moe_parameter_count,
                "training_flops": training_flops,
                "validation_flops": validation_flops,
                "max_ram_mb_train": max_ram_mb_train,
                "max_ram_mb_val": max_ram_mb_val,
            }
            for k in cc_metrics_to_track:
        ...
        # model summary JSON ──────────────────────────────────────────────────
        summary = {
            "f1_mean": float(np.mean(all_final_f1_scores)) if all_final_f1_scores else 0.0,
            "f1_std": float(np.std(all_final_f1_scores)) if all_final_f1_scores else 0.0,
            "accuracy_mean": float(np.mean(all_final_accuracy_scores)) if all_final_accuracy_scores else 0.0,
            "accuracy_std": float(np.std(all_final_accuracy_scores)) if all_final_accuracy_scores else 0.0,
            "training_duration_mean_seconds": float(np.mean(all_training_durations)) if all_training_durations else 0.0,
            "training_duration_std_seconds": float(np.std(all_training_durations)) if all_training_durations else 0.0,
            "validation_duration_mean_seconds": float(np.mean(all_validation_durations)) if all_validation_durations else 0.0,
            "validation_duration_std_seconds": float(np.std(all_validation_durations)) if all_validation_durations else 0.0,
            "parameter_count": int(moe_parameter_count),
...
# Select which to run via
#     experiment.models_to_run="[bitnet,bitnet158b,esc]" or ['1','2','4','8','16',esc]"     

# The script keeps *all* variable names, CSV/JSON summaries, plots, CodeCarbon,
# FLOP count, memory profiling, etc., but removes every MoE-specific element.

# CUDA_VISIBLE_DEVICES=1 python benchmark_baselines.py     --config-path /home/sebastian/codes/repo_clean/QWave/config     --config-name esc50     experiment.datasets.esc.normalization_type=standard     experiment.datasets.esc.csv=/home/sebastian/codes/data/ESC-50-master/VE_soundscapes/efficientnet_1536/esc-50.csv     experiment.device=cuda     experiment.models_to_run="[bitnet]"     experiment.metadata.tag=benchmark_baselines
# """
...
#         return {}
#     df = pd.read_csv(path)
#     # Get the last row if dataframe is not empty, otherwise return empty dict
...
#     if df_full.empty:
#         print(f"[ERROR] Input CSV {csv_path} is empty. Skipping dataset.")
#         return {} # Return empty global_summary
...
#     global_summary = {}
#     all_filtered_rows = [] # List to collect rows for the global combined CSV
...
#         fold_metrics = [] # This list will hold all fold_result dictionaries
...
#         moe_parameter_count = get_num_parameters(dummy_model_for_params)
#         print(f"Params: {moe_parameter_count:,}")
#         del dummy_model_for_params # Delete to free memory
...
#                 "val_dur_s": dur_val,
#                 "params": moe_parameter_count,
#                 "training_flops": training_flops, # Use the specific fold's training FLOPs
#                 "validation_flops": validation_flops, # Use the specific fold's validation FLOPs
#                 "max_ram_mb_train": max_ram_mb_train,
#                 "max_ram_mb_val": max_ram_mb_val,
#             }
#             for k in cc_metrics_to_track:
...
#         # Append to the list for the global combined CSV
#         all_filtered_rows.append(current_model_filtered_data)
